...
function WishList() {
    ...
    const dataFetch = async()=>{
        await fetch('http://localhost:3030/api/getList', {
            method:'POST',
            headers:{
    ...
    async function deleteData(id) {
        ...
        await fetch('http://localhost:3030/api/deleteList', {
            method:'POST',
            headers:{
                'Content-Type':'application/json'
            },
    ...
    // if(localStorage.getItem("wishlist_data")) {
        bookmarkedData = data.map((data) => {
            return (
                <div key={data.id} className="row mb-1">
                    <div className="media pl-2 py-2 w-100 media-row" style={{background: "#222223", border: "none", borderBottom: "1.5px solid #1e1e1e"}}>
                        <img src={data.src} className="img-fluid rounded" style={{width: "100px", height: "100px"}} />
                        <div className="media-body dFlex" style={{height: "100%"}}>
                            <div className="col-8 h-100 px-0" onClick={() => history.push(`/id/${data.id}`)}>
                            ...
                            <div className="col-4 h-100 px-0 mx-0 d-flex delete-btn align-items-center justify-content-center flex-column flex-md-row">
                                ...
                                <div className="col-12 col-md-8 d-flex delete-btn dFlex">
                                    <button className="btn btn-outline-warning" onClick={() => deleteData(data._id)}>Delete</button>
                                </div>
                            </div>
                        </div>
                    </div>
